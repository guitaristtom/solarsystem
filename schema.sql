...
CREATE TABLE `planets` (
  ...
  KEY `uuid_index` (`uuid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Table where the main data for the planets (machines)';


DROP TABLE IF EXISTS `planet_temperatures`;
CREATE TABLE `planet_temperatures` (
  `uuid` varchar(36) NOT NULL COMMENT 'UUID of the "planet" (machine)',
  `chip_name` varchar(32) NOT NULL COMMENT 'Technical name that lm-sensors gives out for your device',
  `temp_number` int(2) NOT NULL COMMENT '0, 1, etc',
  `display_name` varchar(36) NOT NULL COMMENT 'Name that you wish for the adapter to be shown as',
  `last_update` datetime NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT 'DateTime of last update',
  `adapter` varchar(24) NOT NULL COMMENT 'Type of interface the chip is using, "ISA Adapter", "PCI Adapter", etc',
  ...
  KEY `uuid_index` (`uuid`),
  CONSTRAINT `planet_temperatures_ibfk_1` FOREIGN KEY (`uuid`) REFERENCES `planets` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE
